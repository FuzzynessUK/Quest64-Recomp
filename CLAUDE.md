...
- **Audio tab** (2026-09-21) — `src/game/audio.cpp`, `include/audio.h`,
  ...
  - *Custom Music* (2026-09-21, play-tested once as the track_NN.seq
    ...
    control, Randomize Music: Off / On (2026-09-22; Towns / All are gone from
    the UI, `music_shuffle` stays on disk and shows as On). On: a name button
    per track opening one shared paged picker (search box, 60 a page);
    Randomise all (Custom only / Game's own / Mix): jingle slots 28,29,30,43
    draw from `<exe>/fanfares` (names "fanfares/X"), the rest from
    `custom_music`; "game:N" names point a slot at the game's own sequence N
    (original_entry[N]), which is how Game's own shuffles, 44 rows generated
...
## Archipelago rules (2026-09-24)

`src/game/archipelago.cpp`; apworld and yaml generated by
`perl tools/archipelago_world.pl` (copy the apworld to
...
- **Connected, not enabled, is the switch.** Anything that changes play
  ...
  their own location is in the seed (`seed_has`, from the Connected packet's
  checked + missing locations). Unconnected, or sanity off, is vanilla.
- **Seed settings.** The yaml's Randomizer/Enhancements/Cosmetics options
  (table `@seed_settings` in the generator) arrive in slot_data `settings`
  plus `rando_seed`. They can only apply at boot, so `quest64_on_init` calls
  `settle_seed_settings()` first (waits up to 10 s for a connection in
  progress); `randomizer::active_options()` and `enhancements::active_options()`
  then return the seed's. The player connects from the launcher before
  Start. Menus hide while `settings_locked()`.
- Enemy checks: global id = file's first id (0,12,27,38,51,63 for file-table
  rows 0-5) + entry index; bosses from `0x8007D1A0` (gBossData index + 1).
...
- A save read counts as a load only when the Pak menu was opened from the
  main menu (`func_8002B510` a0: 4 = save NPC in the field, 0 = Load).
- Spirit screen = menu mask bit 3 (0x8007B2E4), shared with natural
  level-ups (func_800074A0 0x8000799C). `enhancements::on_frame` closes it
  when every element is at `element_cap()`; spirits are always takeable at
  the cap except under Hard Mode (progressive cap).
