...
- **Enhancements tab** — Fun (One Hit KO, N64 Mode), Quality of Life (JP
  Healing Amounts, JP Magic barrier, Exit from anywhere, Speedrun Timer) and
  Hacks (Landmine36's Hard Mode: the .rup is embedded and applied at boot,
  its payload recompiled, its code patches redone as native hooks; own save
  folder; overrides the randomizer).
...
- A hook cannot sit on a jump's delay slot: N64Recomp emits the slot inside
  ...
  (`UpdateBGM` for the Hard Mode death jingle) or the jump itself instead. A
  hook's text may `return;` to skip the rest of the generated function.
- perl one-liners: `
` inside a replacement string in a double-quoted shell
  argument becomes a literal newline in the C source. Use the Edit tool for
