...
# and the enemy progression data, so the sheet is the ROM's, not typed in.
#
...
my @overview = (
    ...
    [ 'Total', scalar(@chests) + scalar(@givers) + scalar(@monsters) + scalar(@bosses) ],
    [],
...
$z->close;
printf "wrote %s: %d chests, %d givers, %d enemies, %d bosses\n", $out, scalar @chests, scalar @givers, scalar @monsters, scalar @bosses;
