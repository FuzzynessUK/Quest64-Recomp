...
- **Enemy progression** (DOCS/enemyrandologic.xlsx) is DONE 2026-09-20:
  `src/game/randomizer/enemy_progression.cpp`. Files (the six monster sets)
  are the unit of placement because every file loads to one RAM base (file
  table at 0x80054160), so rosters cannot mix files; the sheet's per-monster
  placement is therefore approximated at file level. Stats are scaled at
  battle start by native hooks on the same func_80008FE0 / func_80009818
  sites Hard Mode uses (sheet rules 5-6, incl. the progression guard). The
  data file is generated by `tools/enemyrandologic.pl` together with the
  sheet. Not yet verified in-game: the hook multipliers in a real fight.
